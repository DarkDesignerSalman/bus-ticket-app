var app = Vue.createApp({
    ...
    methods: {
        handleClick(i) {
            let clickdedSeat = this.seats[i]

            if (clickdedSeat.type === 'sold' || clickdedSeat.type === 'booked') {
                alert("You can not selected this seat")
                return
            }
            if (clickdedSeat.type == "available" && this.selectedSeats.length > 2) {
                alert("You can not select more than 3 seats")
                return
            }
            clickdedSeat.type =
                clickdedSeat.type === "selected" ? "available" : "selected"
        },
